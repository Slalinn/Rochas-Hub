-- Rochas Hub V4 - Mobile Walkspeed Fix + Forsaken, Todos ESPs, Quadradinho móvel
-- Corrigido: Walkspeed e JumpPower funcionam no mobile (usando Stepped e CharacterAdded)
-- Inclui: ESP Jogador/NPC/Twisted/Gerador, sliders, switches, animação abas, minimizar quadradinho
...
-- SIDEBAR
local sections = { "Home", "Regretevator", "Dandys World", "Forsaken" }
local icons = { "🏠", "⭐", "🌎", "🧿" }
...
function createModernSlider(parent, y, labelText, default, setter, min, max, step)
    ...
    bar.TouchEnded:Connect(function() dragging = false end)
    ...
    bar.InputChanged:Connect(function(i)
        if dragging and (i.UserInputType == Enum.UserInputType.MouseMovement or i.UserInputType == Enum.UserInputType.Touch) then
            ...
            local px = (i.Position and i.Position.X) or (i.Position and i.Position.X)
            local percent = math.clamp((px-abs)/w,0,1)
...
local function createHome()
    ...
    welcome.Text = "Obrigado por usar o Rochas Hub!\nAproveite as melhores funções de scripts, com ESP, configurações e personalização."
...
local function createRegretevator()
    ...
    createToggleSwitch(page, 38, "Fullbright", false, function(on)
    ...
    -- Fix mobile: Força setar Walkspeed/JumpPower sempre que Humanoid existir e a cada Stepped
    local function setWJ()
        local char = plr.Character
        if char then
            local hum = char:FindFirstChildOfClass("Humanoid")
            if hum then
                if hum.WalkSpeed ~= regreteSpeed then hum.WalkSpeed = regreteSpeed end
                if hum.JumpPower ~= regreteJump then hum.JumpPower = regreteJump end
            end
        end
    end
    game:GetService("RunService").Stepped:Connect(setWJ)
    ...
    createToggleSwitch(page, 150, "ESP Jogadores", false, function(on)
        espEnabled = on
        if espEnabled then
            refreshEsp()
        else
    ...
    createToggleSwitch(page, 190, "ESP NPCs", false, function(on)
        espNPCEnabled = on
    ...
    createToggleSwitch(page, 225, "ESP Geradores", false, function(on)
        espGenEnabled = on
...
local function createDandysWorld()
    ...
    -- Fix mobile: Força setar Walkspeed sempre no Stepped e CharacterAdded
    local function setW()
        local char = plr.Character
        if not char then return end
    ...
    game:GetService("RunService").Stepped:Connect(setW)
    plr.CharacterAdded:Connect(function() setW() end)
    ...
    createToggleSwitch(page, 80, "ESP Twisteds", false, function(on)
    ...
    createToggleSwitch(page, 115, "ESP Geradores", false, function(on)
        espGenEnabled = on
        if not espGenEnabled then
            removeGenEsp()
        end
...
-- NOVA ABA: FORSAKEN
local function createForsaken()
    if pages["Forsaken"] then return end
    local page = Instance.new("Frame", main)
    page.Name = "Forsaken"
    page.Size = UDim2.new(1, -120, 1, -32)
    page.Position = UDim2.new(0, 120, 0, 32)
    ...
    -- Inf Stamina Forsaken
    local infStaminaActive = false
    createToggleSwitch(page, 38, "Inf Stamina", false, function(on)
        infStaminaActive = on
    end)

    -- Script Inf Stamina para Forsaken (Roblox)
    game:GetService("RunService").RenderStepped:Connect(function()
...
-- ANIMAÇÃO DE TRANSIÇÃO DE ABA (desliza para baixo)
...
-- MINIMIZAR = QUADRADO MÓVEL COM IMAGEM
