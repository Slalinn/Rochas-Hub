-- Rochas Hub - Fix: garante que só existe 1 GUI, não trava ao abrir múltiplas vezes!
-- ESP atravessa paredes, Walkspeed Dandys World, Danças, tudo funcional.

local MAX = 5000
local plr = game.Players.LocalPlayer

-- PREVENT MULTIPLE HUBS:
...
main.Position = UDim2.new(0.5, -210, 0.45, -210)
...
local sections = { "Walkspeed", "Jumpsize", "Walkspeed Dandys", "Danças", "Esp", "Fullbright" }
local icons = { "🏃", "🦘", "🌀", "💃", "👁️", "💡" }
...
-- Barra estilizada igual à imagem enviada
...
-- WALK SPEED DANDYS FIXED: sempre reaplica o loop ao respawnar
local wsLoop = nil
local wsConnection = nil
local function setDandysSpeed(v)
	-- função que aplica a velocidade
...
local function createSectionDandysWalkspeed()
	local page = Instance.new("Frame", main)
	page.Name = "Walkspeed Dandys"
	page.Size = UDim2.new(1, -120, 1, -32)
	page.Position = UDim2.new(0, 120, 0, 32)
	page.BackgroundTransparency = 1
	page.Visible = false
	...
	local sectionTitle = Instance.new("TextLabel", page)
	sectionTitle.Text = "| Walkspeed Dandys"
	sectionTitle.Size = UDim2.new(1, 0, 0, 30)
	sectionTitle.Position = UDim2.new(0, 10, 0, 0)
	sectionTitle.Font = Enum.Font.GothamSemibold
	sectionTitle.TextColor3 = Color3.fromRGB(255,255,255)
...
-- ESP (Players + Twisteds: scan workspace for possible monsters)
local function createEsp()
	...
	local subLabel = Instance.new("TextLabel", page)
	subLabel.Text = "ESP Jogadores e Twisteds (Dandys World) - Atravessa paredes"
	...
	local function createEspForPlayer(player)
		...
		box.Transparency = 0.1 -- visível e sempre por cima (atravessa parede)
	...
	-- ESP Twisteds (scan workspace for any monster-like model)
	local espTwistedEnabled = false
	local espTwistedBoxes = {}

	local function removeTwistedEsp()
		for _,v in pairs(espTwistedBoxes) do
			if v and v.Parent then v:Destroy() end
		end
		espTwistedBoxes = {}
	end
	...
	local function createEspForTwisted(twisted)
		...
		box.AlwaysOnTop = true
		box.ZIndex = 11
		box.Size = hrp.Size + Vector3.new(3,3,3)
	...
	game:GetService("RunService").RenderStepped:Connect(function()
		...
		for twisted,box in pairs(espTwistedBoxes) do
			if not twisted.Parent or not isTwistedModel(twisted) then
				box:Destroy()
				espTwistedBoxes[twisted] = nil
			end
		end
...
minBtn.MouseButton1Click:Connect(function()
	...
	main.Size = minimizado and UDim2.new(0,420,0,36) or UDim2.new(0,420,0,420)
end)
